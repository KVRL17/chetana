      ...
      <section className="relative w-full max-w-[1040px] overflow-hidden rounded-[2rem] border border-white/10 bg-white shadow-[0_40px_120px_rgba(0,0,0,.35)] grid lg:grid-cols-[1.05fr_.95fr]">
        <div className="hidden lg:flex bg-[#173f45] text-white p-12 flex-col justify-between min-h-[650px] relative overflow-hidden">
          ...
          <div>
            ...
            <h1 className="mt-8 text-5xl font-semibold leading-[1.05] tracking-[-0.04em]">Every enquiry.<br />One calm workspace.</h1>
            <p className="mt-5 max-w-md text-base leading-7 text-white/70">Review website form submissions, update follow-up status, add private notes, and keep the full enquiry history in your own JSON storage.</p>
          </div>
        ...
        <div className="p-7 sm:p-10 lg:p-12 flex flex-col justify-center">
          <div className="flex items-center gap-3">
            <div className="h-14 w-14 overflow-hidden rounded-2xl border border-[#173f45]/10 shadow-sm">
              <Image src="/chetana-logo.jpeg" width={56} height={56} alt="Chetana" className="h-full w-full object-cover" />
            </div>
          ...
          <div className="mt-10">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">Sign in to view and manage website enquiries.</p>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-slate-950/35 backdrop-blur-sm flex justify-end" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <aside className="h-full w-full max-w-2xl bg-white shadow-[-30px_0_80px_rgba(15,23,42,.18)] flex flex-col animate-[slideIn_.2s_ease-out]">
        ...
        <div className="border-t border-slate-100 bg-white px-5 py-4 sm:px-7 flex items-center justify-between gap-3">
          <div className="text-xs text-slate-400">Changes update the JSON file immediately.</div>
...
export default function AdminDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#f4f5f3] text-slate-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[278px] bg-[#12373d] text-white transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-full flex-col p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3"><div className="h-11 w-11 overflow-hidden rounded-xl border border-white/15"><Image src="/chetana-logo.jpeg" width={44} height={44} alt="Chetana" className="h-full w-full object-cover" /></div><div><div className="text-sm font-bold">Chetana</div><div className="text-[11px] text-white/50">Admin Console</div></div></div>
      ...
      <section className="min-h-screen lg:pl-[278px]">
        <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-[#f4f5f3]/90 backdrop-blur-xl">
          <div className="flex h-[76px] items-center justify-between gap-4 px-4 sm:px-7 lg:px-9">
            <div className="flex items-center gap-3"><button onClick={() => setSidebarOpen(true)} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white lg:hidden"><Menu className="h-4 w-4" /></button><div><h1 className="text-lg font-semibold tracking-[-0.025em] text-slate-900">{routedView.title}</h1><p className="hidden text-xs text-slate-500 sm:block">{routedView.subtitle}</p></div></div>
            <div className="flex items-center gap-2"><button onClick={loadRecords} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50" aria-label="Refresh"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /></button><button onClick={downloadJson} className="hidden h-10 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 text-xs font-bold text-slate-700 hover:bg-slate-50 sm:inline-flex"><Download className="h-4 w-4" /> Export JSON</button><div className="ml-1 grid h-10 w-10 place-items-center rounded-xl bg-[#173f45] text-xs font-bold text-white">AD</div></div>
          </div>
        </header>
      ...
      {selected && <DetailPanel record={selected} onClose={() => setSelected(null)} onSaved={handleSaved} />}
    </main>
  );
}
