...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  return (
    <PageWrapper>
      ...
      <article>
        <header className="relative overflow-hidden bg-[linear-gradient(135deg,#edf2ef,#f5eee4)]"><div className="container-shell max-w-4xl py-11 text-center sm:py-20"><div className="eyebrow mb-5">{post.category}</div><h1 className="text-[clamp(2.25rem,10vw,3.15rem)] sm:text-[clamp(2.4rem,5vw,4.6rem)] font-[760] leading-[1.03] text-primary text-balance">{post.title}</h1><p className="lead-copy mx-auto mt-6 max-w-2xl">{post.excerpt}</p><div className="mt-6 flex flex-wrap items-center justify-center gap-3 text-[10.5px] font-bold text-muted sm:mt-7 sm:gap-4 sm:text-xs"><span className="flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5" />{new Date(`${post.publishedAt}T00:00:00`).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</span><span className="flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5" />{post.readingTime}</span></div></div></header>
